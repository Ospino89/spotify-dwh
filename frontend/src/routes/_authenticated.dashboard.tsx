...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — My Spotify Wrapped" }] }),
...
function DashboardPage() {
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-4 text-base text-foreground">
          Your data warehouse is empty. Go to ETL tab and sync your data.
        </p>
        <Link
          to="/etl"
          className="mt-6 rounded-full border border-primary px-5 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
        >
          Go to ETL
        </Link>
      </div>
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">Your listening, at a glance</h1>
        <p className="mt-1 text-sm text-muted-foreground">Last 4 weeks of activity</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
              ...
              <li key={a.name} className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <span className="text-xs tabular-nums text-muted-foreground">{a.popularity}</span>
                  </div>
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                    <div className="h-full rounded-full bg-primary" style={{ width: `${a.popularity}%` }} />
                  </div>
                </div>
              </li>
            ))}
        ...
        <Card title="Peak Listening Hour">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Your most active hour of the day</p>
          <div className="mt-6 flex h-24 items-end gap-1">
            {hours.map((v, i) => (
              <div
                key={i}
                className={`flex-1 rounded-sm ${i === peakIdx ? "bg-primary" : "bg-secondary"}`}
            ...
            {genres.map((g) => (
              <li key={g.name}>
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  <span className="text-xs tabular-nums text-muted-foreground">{g.plays.toLocaleString()} plays</span>
                </div>
                <div className="h-2 w-full overflow-hidden rounded-full bg-secondary">
                  <div className="h-full rounded-full bg-primary" style={{ width: `${(g.plays / maxGenre) * 100}%` }} />
                </div>
              </li>
            ))}
          </ul>
